...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**AFK 🐨!**\nibal nya pergi dulu ya ayang...\
        \nReason: `{string}`")
    else:
        await afk_e.edit("**AFK 🐨!**\nibal nya pergi dulu ya sayang...")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**OK Iball Udah ON....**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"ibal pergi {afk_since} Yg Lalu.\
                        \nReason: `{AFKREASON}`")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"ibalnya gaada ASUU... {afk_since}.\
                        \nReason: `{AFKREASON}`")
                else:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(f"dibilang ibalnya gaada... {afk_since}.\
                            \nReason: `{AFKREASON}`")
                    else:
                        await sender.reply(str(choice(AFKSTR)))
